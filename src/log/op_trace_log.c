...
void init_op_trace_log(void) {
    if (op_asm_log_file == NULL) {
        op_asm_log_file = file_tag_fopen(OUTPUT_DIR, "op_log_full_asm.txt", "w");
        if (op_asm_log_file == NULL) {
            perror("Error opening op_log_full_asm.txt in output directory");
        } else {
            atexit(close_op_asm_log_file);
        }
    }
    if (retired_op_log_file == NULL) {
        retired_op_log_file = file_tag_fopen(OUTPUT_DIR, "retired_op_per_cycle.txt", "w");
        if (retired_op_log_file == NULL) {
            perror("Error opening retired_op_per_cycle.txt in output directory");
        } else {
...
void log_issued_op(Op* op, Counter cycle_count) {
    if (op_asm_log_file && cycle_count >= log_start_cycle && cycle_count <= log_end_cycle) {
        const char* disasm_str = disasm_op(op, TRUE);
        char func_op_str[MAX_STR_LENGTH * 4];
        sprint_func_op(func_op_str, op);

        if (op->table_info->cf_type) {
            fprintf(op_asm_log_file, "Cycle:%-10llu OpNum:%-10llu PC:0x%-10llx OffPath:%d Disasm: %-40s Conf:%d Target:0x%-10llx Dir:%d PredNPC:0x%llx Pred:%d\n",
                    cycle_count,
                    op->op_num,
                    op->inst_info->addr,
                    op->off_path,
...
void log_retired_ops(Counter cycle_count, uns ret_count) {
    if (retired_op_log_file && cycle_count >= log_start_cycle && cycle_count <= log_end_cycle) {
        if (ret_count > 0) {
